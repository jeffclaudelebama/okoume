...
const navigation = [["Accueil", "⌂"], ["Rayons", "⊞"], ["Offres", "✦"], ["Commandes", "□"], ["Compte", "◯"]] as const;
...
export function StoreShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-dvh bg-white text-black dark:bg-[#0A0A0A] dark:text-white">
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto grid h-[66px] max-w-2xl grid-cols-5 border-t border-[#E5E5E5] bg-white/95 px-2 backdrop-blur dark:border-[#262626] dark:bg-[#0A0A0A]/95" aria-label="Navigation principale">{navigation.map(([label, icon], index) => <button type="button" key={label} className={`flex flex-col items-center justify-center gap-0.5 text-[10px] font-medium ${index === 0 ? "text-black dark:text-white" : "text-neutral-500 dark:text-neutral-400"}`}><span className="text-lg leading-none" aria-hidden="true">{icon}</span>{label}</button>)}</nav>
  </div>;
}
